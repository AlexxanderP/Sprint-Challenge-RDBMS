...
RDBM (Relational Database Management System) is the database software itself, which manages storage, querying, updating, etc.
SQL (Structured Query Language), is a language which is typically used to query the RDBMS.

1. Why do tables need a `primary key`?

Primary Keys uniquely identify each record/row in the table. It is required in order to define a Foreign Key relationship between parent and child tables.
